...
export const GroceryProvider = ({ children }: GroceryProviderProps) => {
  ...
  const downloadListAsPdf = async (listId: string) => {
    ...
    try {
      ...
      // Use a standardized approach with no external font loading
      // We'll use the default font which has better compatibility
      ...
      doc.setFontSize(10);
      doc.text(`Created: ${createdDate}`, 20, 40);
      
      // Add BazarBuddy logo/branding
      doc.setFontSize(10);
      doc.setTextColor(150, 150, 150);
      doc.text("Generated by BazarBuddy", 20, 45);
      
      // Import and use autoTable
      const { default: autoTable } = await import('jspdf-autotable');
      
      // Add items table - using simplified font approach
      autoTable(doc, {
        startY: 55,
